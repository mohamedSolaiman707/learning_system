  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Row(
        children: [
          if (!isMobile)
            Container(width: 250, color: Colors.white, child: _buildDrawer(context, isDrawer: false)),
          Expanded(
            child: _isLoading 
        ...
        if (isDrawer)
          const DrawerHeader(
            ...
            child: Column(
              ...
              children: [Icon(Icons.school, color: Colors.white, size: 48), SizedBox(height: 10), Text("نظام الإدارة", style: TextStyle(color: Colors.white, fontSize: 18))],
            ),
          ),
        ListTile(leading: const Icon(IconlyLight.chart), title: const Text("لوحة التحكم"), onTap: () {}, selected: true),
        ListTile(leading: const Icon(IconlyLight.user_1), title: const Text("إدارة المستخدمين"), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const UsersManagementScreen()))),
        ListTile(leading: const Icon(IconlyLight.setting), title: const Text("الإعدادات"), onTap: () {}),
        const Divider(),
        ListTile(leading: const Icon(IconlyLight.logout, color: Colors.red), title: const Text("تسجيل الخروج", style: TextStyle(color: Colors.red)), onTap: () => supabase.auth.signOut().then((_) => Navigator.pushReplacementNamed(context, '/login'))),
      ],
    );
  }
}
