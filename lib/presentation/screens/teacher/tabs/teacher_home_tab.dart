...
class _TeacherHomeTabState extends State<TeacherHomeTab> {
  ...
  Future<void> _loadTeacherData() async {
    ...
    try {
      ...
      // جلب جميع حصص المدرس التي لم تنتهِ بعد (UTC Comparison)
      ...
      // حساب إجمالي الطلاب المسجلين في هذه الحصص
  ...
  Widget build(BuildContext context) {
    ...
    final String? currentClassCode = currentSession == null ? null : _allSessionsRaw.firstWhere((s) => s['id'] == currentSession.id)['class_code'];
  ...
  Widget _buildCurrentSessionCard(SessionModel session, String? classCode) => Container(
    ...
    child: Column(children: [
      Row(children: [const Icon(IconlyLight.video, color: Colors.white, size: 28), const SizedBox(width: 16), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(session.subjectName, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)), Text("${DateFormat('hh:mm a').format(session.startTime)} - ${DateFormat('hh:mm a').format(session.endTime)}", style: TextStyle(color: Colors.white.withOpacity(0.8)))]))]), 
      if (classCode != null) ...[const SizedBox(height: 20), Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white12, borderRadius: BorderRadius.circular(12)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("كود: $classCode", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), IconButton(onPressed: () { Clipboard.setData(ClipboardData(text: classCode)); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("تم نسخ الكود"))); }, icon: const Icon(Icons.copy, color: Colors.white, size: 18))]))], 
      const SizedBox(height: 24), 
      Row(children: [
  ...
  Widget _buildLoadingSkeleton() => Shimmer.fromColors(baseColor: Colors.grey.shade300, highlightColor: Colors.grey.shade100, child: Padding(padding: const EdgeInsets.all(20), child: Column(children: [Container(height: 100, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20))), const SizedBox(height: 20), Container(height: 200, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20)))])));
