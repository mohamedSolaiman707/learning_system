...
serve(async (req) => {
  ...
  try {
    ...
    // قراءة الـ body كـ text للتحقق من التوقيع
    ...
    // عندما ينتهي التسجيل تماماً
    if (event.event === 'egress_ended') {
      const egressId = event.egressInfo?.egressId
      
      if (egressId) {
        // تحديث حالة الفيديو في الداتابيز ليكون متاحاً للطالب
        ...
        // تحديث حالة الحصة الأصلية إنها لم تعد تسجل
